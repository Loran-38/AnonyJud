...
# Import pdf-redactor pour anonymisation avancée avec préservation de mise en page
try:
    import pdf_redactor
    PDF_REDACTOR_AVAILABLE = True
    print("✓ pdf-redactor disponible pour anonymisation PDF avancée")
except ImportError as e:
...
def anonymize_pdf_with_redactor(pdf_content: bytes, tiers: List[Dict[str, Any]] = []) -> Tuple[bytes, Dict[str, str]]:
    """
    Anonymise un PDF avec pdf-redactor pour préserver parfaitement la mise en page.
    ...
    try:
        ...
        # Ajouter les remplacements des tiers (utilisation du numéro fixe comme dans la mémoire)
        for tier in tiers:
